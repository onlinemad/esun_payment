...
/**
* @param {string} sid 系統廠商代號
* @param {Array} stores 待請款的特店
* @returns {Buffer} 請款檔 Big5 Buffer
*/
esun.statement = function (sid, stores) {
  ...
  stores.map(stroe => {
    ...
    stroe.orders.map(order => {
      sub_data += `FD${rightPad(order.id, 20, ' ')}${order.type}${leftPad(order.amount.toFixed(2).replace('.', ''), 12, '0')}  ${rightPad('', 25, ' ')}${rightPad('', 20, '　')}${rightPad('', 20, ' ')}${leftPad('', 25, ' ')}${eol}`
    })
  ...
  return iconv.encode(`${header}${body}${trailer}`, 'big5')
}
